...
#ifdef _DEBUG
	vsBlob = LoadCSO("Shaders/compiled/Debug/VS_OldMainRenderPass.cso");
	psBlob = LoadCSO("Shaders/compiled/Debug/PS_OldMainRenderPass.cso");
#else
	vsBlob = LoadCSO("Shaders/compiled/Release/VS_OldMainRenderPass.cso");
	psBlob = LoadCSO("Shaders/compiled/Release/PS_OldMainRenderPass.cso");
#endif // _DEBUG
...
OldMainRenderPass::~OldMainRenderPass()
{
	for (auto& t : m_constantBuffers)
	{
		for (auto& cbManager : t)
...
void OldMainRenderPass::Start(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList)
{

}

void OldMainRenderPass::SubmitObjectsToRender(const std::vector<RenderUnit>& renderUnits)
{
...
void OldMainRenderPass::RunRenderPass(std::vector<ID3D12GraphicsCommandList*> cmdLists, std::vector<DescriptorHandle> descriptorHandles, FrameResource& frameResource, int frameIndex)
{
...
{
	return "OldMainRenderPass";
}

void OldMainRenderPass::UpdateDynamicLights(int frameIndex)
{
